...
const token = "thr1.AAAAAGjEUkWYkRZ0qhScDg.pcnlOd7skYg"; // https://www.haxball.com/headlesstoken

var BKWebhook = 'https://corsproxy.io/?https://discord.com/api/webhooks/1416120050024317021/P1jOS997YwkPKFkMjsfLi6NWz_q57vSA2n_YkydOZhLEnsuR6TeW6ZNZ5MjDfH_28x5N';
var roomWebhook = 'https://corsproxy.io/?https://discord.com/api/webhooks/1416094377654353931/Kk-Wmd9Q5Zi1jH2WqGu3kbcc456iniQwd2oDRwLmUIxW0U5GVI6l0fbmy5m09l5zsFV9'; // Eventos
var callsWebhook = 'https://corsproxy.io/?https://discord.com/api/webhooks/1407420909479792804/qBtkEtfPK-lBdg3_C2dzIYr4GRF3X31ArnmsPj-b-hKlLFEKtYegJkhBQY3IPM1I9W3H'; // LLamadas administrador
var streakWebhook = 'https://corsproxy.io/?https://discord.com/api/webhooks/1406800855763652720/lrBf9yHxiYQpzDrsD1T4tLBEJC-WdYxmu09BQmBBtGmnynvV1fBvkj1kB7vx-WWsDR1W'; // Rachas
var gameWebhook = 'https://corsproxy.io/?https://discord.com/api/webhooks/1416093231212396735/vU35dmmvyRuimfRvYBj6Tyr_5RRuyAv89dsMMGhH_AcxfGA6tZintmx5tC7v-ywTj76a'; // Grabaciones de partidas
var errorWebhook = 'https://corsproxy.io/?https://discord.com/api/webhooks/1407429834635739146/GnkY6_p1yzjSX8zgWYddjLBntegvBHrwaWu1YgffH3QIERq6eWXxTW7VucgAR_Ko78aU' // Errores
var bansWebhook = 'https://corsproxy.io/?https://discord.com/api/webhooks/1408194790322999296/1S9w9OT8MZTcz2Pb_4S2XTSXcdq1iH7M-jEIjDUTVCdKd-8tiydofheIAF_6UliCzulE'; // Baneos
var fetchRecordingVariable = true;
var timeLimit = 4;
var scoreLimit = 3;
var discordlink = 'https://discord.gg/3jFY7Ma74U'
var idadminrol = '1416093096932016209' // Id del rol de administrador de esta sala
...
class WebhookQueue {
        ...
        try {
            ...
            if (response.status === 429) {
                ...
                if (this.errorCount >= this.maxErrors) {
                    this.stop();
                    await this.reportError("⚠️ Demasiados errores consecutivos. Sistema detenido.");
                }
            } else if (!response.ok) {
                this.errorCount++;
    ...
    send(player, message, type = "chat", extra = {}) {
        ...
        if (this.queue.length >= this.maxQueueSize) {
            this.reportError(`⚠️ Cola saturada (${this.queue.length}), mensaje descartado.`);
            return;
        }
    ...
    stop() {
        ...
        this.queue = [];
        console.log("🛑 Sistema de Webhook detenido por errores.");
    }
    ...
    restart() {
        ...
        this.errorCount = 0;
        this.queue = [];
        console.log("✅ Sistema de Webhook reactivado manualmente.");
    }
}
...
async function backup(mode){
    const webhookUrl=BKWebhook,intervalHours=12;
    try{
        const now=new Date();
        if(lastBackup&&(now-lastBackup)<intervalHours*3600*1000)return;
        const data={};
        ...
        const date=new Date();
        const fileName=`${mode}-Backup-${roomName}-${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}_${String(date.getHours()).padStart(2,'0')}-${String(date.getMinutes()).padStart(2,'0')}.json`;
        const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
        const formData=new FormData();
        formData.append('file',blob,fileName);
        const response=await fetch(webhookUrl,{method:'POST',body:formData});
        if(!response.ok)throw new Error(`Error enviando el archivo a Discord: ${response.statusText}`);
        lastBackup=now;
    }catch(error){
...
function calculateStadiumVariables() {
    if (checkStadiumVariable && teamRed.length + teamBlue.length > 0) {
        ...
        setTimeout(() => {
            ...
            const playerDisc = firstPlayer ? room.getPlayerDiscProperties(firstPlayer.id) : null;
            const defaultBallRadius = 5.8;
            const defaultPlayerRadius = 15;
            ballRadius = ballDisc?.radius ?? defaultBallRadius;
            playerRadius = playerDisc?.radius ?? defaultPlayerRadius;
            triggerDistance = ballRadius + playerRadius + 0.01;
            const invMass = ballDisc?.invMass ?? 1;
            const damping = ballDisc?.damping ?? 0;
            speedCoefficient = 100 / (5 * invMass * ((damping ** 60) + 1));
        }, 1);
    }
}
...
function alternarSistema(player) {
    if (webhookQueue.isOperational) {
        webhookQueue.stop();
        room.sendAnnouncement("✅ Sistema desactivado manualmente.", player.id, warningColor, 'bold', HaxNotification.CHAT);
    } else {
        webhookQueue.restart();
        room.sendAnnouncement("✅ Sistema reactivado manualmente.", player.id, warningColor, 'bold', HaxNotification.CHAT);
    }
}
        ...
        if (teamRed.length <= teamBlue.length && player.id == teamRed[0].id) {
            if (['top', 'auto'].includes(msgArray[0].toLowerCase())) {
                ...
                if (Number.parseInt(msgArray[0]) > teamSpec.length || Number.parseInt(msgArray[0]) < 1) {
                    ...
                    room.sendAnnouncement(
                        `✅ ${player.name} eligió a ${teamSpec[Number.parseInt(msgArray[0]) - 1].name}!`,
                        null,
                        announcementColor,
                        'bold',
                        HaxNotification.CHAT
...
function updatePlayerStats(player, teamStats) {
    ...
    // --- Cálculo ELO ---
    let baseElo = (lastWinner == teamStats) ? 30 : -25;
    ...
    stats.elo = Math.max(1000, (stats.elo || 1000) + eloChange);
...
function narrateGoal(team) {
    try {
        const scores = game.scores;
        const atribucion = getGoalAttribution(team); 
        const striker = atribucion[0];
        const assist = atribucion[1];
        const teamName = team === Team.RED ? 'rojo' : team === Team.BLUE ? 'azul' : 'desconocido';
        const time = getTimeGame(scores.time);
        let narration = '';

        if (!striker) {
            narration = `[🎤] ${time} ¡Gol para el equipo ${teamName}!`;
            room.sendAnnouncement(narration, null, 0xA3FF00, null, HaxNotification.CHAT);
            return;
        }

        const isOwnGoal = striker.team !== team;

        if (isOwnGoal) {
            const ownGoalNarrations = [
                `[🎤] ${time} ¡Increíble! ${striker.name} mete en propia puerta.`,
                `[🎤] ${time} Autogol de ${striker.name}, qué mala fortuna.`,
                `[🎤] ${time} ¡Error garrafal! ${striker.name} anota en contra.`,
                `[🎤] ${time} El balón termina dentro por ${striker.name}... ¡Autogol!`
            ];
        ...
        if (assist && assist.team === team) {
            const assistNarrations = [
                `[🎤] ${time} ¡Gol de ${striker.name}! Gran pase de ${assist.name}.`,
                `[🎤] ${time} ${assist.name} asiste, ${striker.name} define y ¡golazo!`,
                `[🎤] ${time} ¡Perfecta combinación entre ${assist.name} y ${striker.name}!`,
                `[🎤] ${time} ${striker.name} anota tras asistencia de ${assist.name}.`
            ];
            narration = assistNarrations[Math.floor(Math.random() * assistNarrations.length)];
            room.sendAnnouncement(narration, null, 0xA3FF00, null, HaxNotification.CHAT);
            return;
        }

        const soloNarrations = [
            `[🎤] ${time} ¡Gol de ${striker.name}!`,
            `[🎤] ${time} ${striker.name} marca para el equipo ${teamName}.`,
            `[🎤] ${time} ¡Qué disparo de ${striker.name}, golazo!`,
            `[🎤] ${time} ${striker.name} convierte y suma para el ${teamName}.`
        ];
        narration = soloNarrations[Math.floor(Math.random() * soloNarrations.length)];

        room.sendAnnouncement(narration, null, 0xA3FF00, null, HaxNotification.CHAT);

    } catch (err) {
        console.error('Error en narrateGoal:', err);
        room.sendAnnouncement(
            `[⚠️] Error al narrar el gol.`,
            null, 0xFF0000, null, HaxNotification.CHAT
        );
    }
}
